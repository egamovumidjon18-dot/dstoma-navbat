...
export default function ClientDashboard({
  ...
  // Registration collects only fullName + password now; birthDate/bloodGroup/
  // allergies/chronicDiseases used to also default here but were never sent
  // anywhere — those fields are deferred to PatientPanel's Sozlamalar tab,
  // filled in by the patient after registering. hasInfection is the one
  // exception still sent at registration (see newPatient below); it also
  // has no way to be set to true here, so it's editable from Sozlamalar too.
  const [hasInfection] = useState<boolean>(false);
