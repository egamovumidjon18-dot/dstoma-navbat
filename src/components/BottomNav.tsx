...
interface BottomNavProps {
  ...
  onBook: () => void;
  onOpenMore: () => void;
  // Optional so any other caller can keep working unchanged; PatientPanel
  // passes its own t() so labels follow the patient's language setting.
...
export default function BottomNav({ activeTab, onNavigate, onBook, onOpenMore, t = (s) => s }: BottomNavProps) {
  ...
  return (
    ...
    >
      {item('bosh_sahifa', <Home size={20} />, t("bosh sahifa"), () => onNavigate('bosh_sahifa'))}
      {item('navbatlar', <Calendar size={20} />, t("navbatlar"), () => onNavigate('navbatlar'))}
      ...
      <div className="flex-1 flex items-center justify-center relative">
        <button
          ...
          aria-label={t("navbat olish")}
        >
          <Plus size={26} />
        </button>
      </div>

      {item('tish_sxemasi', <Smile size={20} />, t("tish sxemasi"), () => onNavigate('tish_sxemasi'))}
      ...
      >
        <User size={20} />
        <span className="text-[10px] font-bold">{t("profil")}</span>
      </button>
    </nav>
  );
}
